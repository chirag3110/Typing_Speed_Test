...
def check_input(window,canvas1,write,words_text):
    ...
    try_again=Button(text="Try Again",command=partial(again,window),font=("Helvetica",13,"bold italic"),bg="#c80815",fg="white",relief="raised")
...
def foo(window):
    ...
    bg=Image.open("bg2ts.jpg")
    bg=bg.resize((600,450))
    bg=ImageTk.PhotoImage(bg)
    canvas1.create_image(0,0,image=bg,anchor=NW)

    icon=Image.open("iconst.png")
    ...
    start_btn=Button(text="Start",command=partial(start,mins,sec,screen,canvas1,write,words_text),font=("Calibri",18,"bold"),width=5,bg="#c80815",fg="white",relief="raised")
...
def main():
    ...
    bg=Image.open("bgst.jpg")
    bg=bg.resize((600,320))
    bg=ImageTk.PhotoImage(bg)
    canvas1=Canvas(height=400,width=600)
    canvas1.create_image(0,0,image=bg,anchor=NW)
    canvas1.pack()
    go_btn=Button(text="Proceed",font=("Helvetica",18,"bold italic"),width=8,bg="#c80815",fg="white",relief="raised",command=partial(foo,window))
    exit_btn=Button(text="Exit",font=("Helvetica",19,"bold italic"),width=5,bg="#c80815",fg="white",relief="raised",command=window.destroy)
